...
# Steam 评分缓存文件路径
STEAM_RATING_CACHE_FILE = os.path.join(os.path.dirname(__file__), 'data', 'steam_rating_cache.json')
STEAM_RATING_CACHE_TTL = 24 * 60 * 60  # 24小时缓存

def load_steam_rating_cache():
    if os.path.exists(STEAM_RATING_CACHE_FILE):
        try:
            with open(STEAM_RATING_CACHE_FILE, 'r', encoding='utf-8') as f:
                return json.load(f)
        except:
            pass
    return {}

def save_steam_rating_cache(cache):
    os.makedirs(os.path.dirname(STEAM_RATING_CACHE_FILE), exist_ok=True)
    with open(STEAM_RATING_CACHE_FILE, 'w', encoding='utf-8') as f:
        json.dump(cache, f, ensure_ascii=False, indent=2)

def get_cached_steam_rating(app_id):
    cache = load_steam_rating_cache()
    if app_id in cache:
        cached_data = cache[app_id]
        cached_time = cached_data.get('cached_at', 0)
        if datetime.now().timestamp() - cached_time < STEAM_RATING_CACHE_TTL:
            return cached_data
    return None

def set_cached_steam_rating(app_id, data):
    cache = load_steam_rating_cache()
    data['cached_at'] = datetime.now().timestamp()
    cache[app_id] = data
    save_steam_rating_cache(cache)
...
def get_steam_rating():
    """获取 Steam 游戏好评率 - 使用 appreviews API（24小时内缓存有效）"""
    ...
    try:
        ...
        # 第1步：先检查缓存（24小时内有效）
        cached = get_cached_steam_rating(app_id)
        if cached:
            # 缓存有效，直接返回
            cached['cached'] = True
            return jsonify(cached)
        
        # 第2步：缓存没有或过期，查询 Steam API
        try:
            ...
            with urllib.request.urlopen(req, timeout=5) as response:
                ...
                if data.get('success'):
                    ...
                    # 保存到缓存
                    set_cached_steam_rating(app_id, result.copy())
                    
                    return jsonify(result)
                else:
                    return jsonify({'success': False, 'error': '无法获取游戏评价'})
        except Exception as e:
            # Steam API 查询失败/超时，不设置缓存，直接返回错误
            return jsonify({'success': False, 'error': '查询 Steam 失败: ' + str(e)})
